...
const ex1V3 = {
  ...
  rule: 'Accept a randomly chosen neighbor only if strictly better. Sequence: 1, 0, 1, 2, 3, 2, 3.',
  graphPoints: [
    { x: 0, label: 'x=0' },
    { x: 1, label: 'x=1' },
    { x: 1, label: 'x=1' },
  ...
  steps: [
    ...
    step('x = 3  |  f(3) = 9',
      ['Random draw: try x = 2',
        'f(2) = −16 + 64 − 72 + 24 = 0',
        '0 < 9  ✗  →  not strictly better'],
      'STAY at x = 3', 'reject'),
    step('x = 3  |  f(3) = 9',
      ['Random draw: try x = 3',
        'f(3) = 9  →  equal, not strictly better',
        'Random sequence exhausted'],
      'STOP  (sequence exhausted  —  goal f = 20 NOT reached)', 'stop'),
  ],
  result: { text: 'x = 3,  f(x) = 9', reached: false, note: 'Escaped the valley by randomly jumping to x = 3 — but sequence ended before reaching a higher peak' },
};
...
const ex3Steps = [
  ...
  step('x = 3  |  f(3) = 1  |  t = 30 ms',
    ['Check x + 1 = 4:  f(4) = 16 − 24 + 10 = 2',
      ...
      'Check x − 1 = 2:  f(2) = 4 − 12 + 10 = 2',
      '2 > 1  ✗  →  not lower',
      'No improving neighbor  |  Time used: 50 ms of 60 ms  (time was NOT the factor)'],
...
export const exercises = [
  {
    ...
    problem: {
      ...
      note: 'Apply all three variants — Simple, Steepest Ascent, and Stochastic — starting from x = 0. For Stochastic Hill Climbing, the random neighbors generated in order are: x = 1, 0, 1, 2, 3, 2, 3.',
    },
    hasVariants: true,
  ...
  },
  {
    ...
    followUp: 'If the goal is not reached, at which point did the algorithm terminate? What was the value of Y(i) at that point?',
  },
  {
    id: 5,
    title: 'Exercise 5',
